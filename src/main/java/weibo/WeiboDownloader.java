...
public class WeiboDownloader {
	 ...
	 * 路径
	 * */
	public static String IMG_LOCATION = "I:\\Download\\wbdown";
	...
	public static void downloadCli(String selectType, String inputName, String filePath, String startTime, String endTime) throws ParseException, IOException, InterruptedException {
		...
		List<VoUrlAndName> voUrlAndNames;
		try {
			voUrlAndNames = WeiboUtils.getAllImgURL(containerId);
		} catch (Exception e1) {
			e1.printStackTrace();
		...
		System.out.println("图片数量: " + voUrlAndNames.size());
		...
		CountDownLatch downLatch = new CountDownLatch(voUrlAndNames.size());
		ExecutorService executor = Executors.newFixedThreadPool(4);
		for(int i=0;i<voUrlAndNames.size();i++){
			executor.submit(new ImageDownloadTask(downLatch, i,voUrlAndNames.get(i).getFilename(), voUrlAndNames.get(i).getUrl()));
		}
